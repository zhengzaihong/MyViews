...
public class FilletView extends AppCompatTextView {
    ...
    public FilletView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // 读取属性值
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.FilletView);
        configBean.setNormalBgColor(ta.getColor(R.styleable.FilletView_f_normalBgColor, Color.TRANSPARENT));
        configBean.setPressedBgColor(ta.getColor(R.styleable.FilletView_f_pressedBgColor, configBean.getPressedBgColor()));
        configBean.setStrokeWidth(ta.getDimensionPixelSize(R.styleable.FilletView_f_strokeWidth, 0));
        configBean.setStrokeColor(ta.getColor(R.styleable.FilletView_f_strokeColor, Color.TRANSPARENT));
        configBean.setCornerRadius(ta.getDimensionPixelSize(R.styleable.FilletView_f_cornerRadius, 0));
        configBean.setFollowTextColor(ta.getBoolean(R.styleable.FilletView_f_followTextColor, false));

        configBean.setNormalTextColor(ta.getColor(R.styleable.FilletView_f_normalTextColor, configBean.getNormalTextColor()));
        configBean.setPressedTextColor(ta.getColor(R.styleable.FilletView_f_pressedTextColor, configBean.getPressedTextColor()));
        configBean.setShowAnimation(ta.getBoolean(R.styleable.FilletView_f_showAnimation, false));

        int radiusType = ta.getInt(R.styleable.FilletView_f_radius_type, 0);

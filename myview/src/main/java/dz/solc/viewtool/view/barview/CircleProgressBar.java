    ...
    public CircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.circleProgressBar_firstColor) {
                ...
                secondColor = ta.getColor(attr, Color.WHITE); //

            } else if (attr == R.styleable.circleProgressBar_progress) {
                currentValue = ta.getIndex(attr); // 默认进度

            } else if (attr == R.styleable.circleProgressBar_showText) {
                showText = ta.getBoolean(attr, true); // 默认进度
...
//        circlePaint.setShadowLayer(10, 10, 10, Color.RED);
     ...
     *
     ...
     app:circleWidth="6dp"
     app:firstColor="#60ffffff"
     app:progress="10"
     app:secondColor="#ffffff"
     app:showText="false

     使用方法
     circleProgressBar.setSecondColor(xxxxx);
     circleProgressBar.setProgress(88, true);
